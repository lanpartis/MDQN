...
class DQNAgent:
    ...
    def build_model(self):
        model = Sequential()
        model.add(Conv2D(filters=n_s[0],kernel_size=(fil_size[0],fil_size[0]),strides=(st[0],st[0]),padding='valid',input_shape=input_shape,initializer= RandomNormal(0,0.5),activation='relu'))
        model.add(MaxPooling2D(pool_size=(p_s,p_s),strides=(p_s,p_s),padding='same'))
        model.add(Conv2D(filters=n_s[1],kernel_size=(fil_size[1],fil_size[1]),strides=(st[1],st[1]),initializer= RandomNormal(0,0.5),padding='valid',activation='relu'))
        model.add(MaxPooling2D(pool_size=(p_s,p_s),strides=(p_s,p_s),padding='same'))
        model.add(Conv2D(filters=n_s[2],kernel_size=(fil_size[1],fil_size[1]),strides=(st[1],st[1]),initializer= RandomNormal(0,0.5),padding='valid',activation='relu'))
        model.add(MaxPooling2D(pool_size=(p_s,p_s),strides=(p_s,p_s),padding='same'))
        model.add(Flatten())
        model.add(Dense(n_s[3],activation='relu',initializer= RandomNormal(0,0.5)))
        model.add(Dense(outpur_shape,initializer= RandomNormal(0,0.5)))
